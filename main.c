...
int main(int argc, char **argv) {
    ...
    printf(" > Thread terminated! Terminating main...\n");
...
void *consumer(void) {
    ...
    for (int i = 0; i<LOOP_TIMES; i++) {
        ...
        // print information
        if (data.buf == URGENT) {
            printf(" >>> Consumer: ms is %d and buffer is URGENT!", data.ms);
        } else {
            printf(" >>> Consumer: ms is %d and buffer is NORMAL!", data.ms);
        }
    }
    return NULL;
}
...
void *producer(void) {
    ...
    for (int i = 0; i<LOOP_TIMES; i++) {
        ...
        // print data
        if (buf == URGENT) {
            printf(" >> Producer: ms is %d and buffer is URGENT!", (int) ms);
            d.buf = URGENT;
        } else {
            printf(" >> Producer: ms is %d and buffer is NORMAL!", (int) ms);
            d.buf = NORMAL;
        }
        d.ms = (int) ms;
